...
Mesh* Mesh::GenerateTriangle() {
	...
	m->vertices[1] = Vector3(0.5f, -0.5, 0.0f);
...
Mesh* Mesh::GenerateSquare() {
	...
	m->textureCoords[3] = Vector2(0.0f, 0.5f);
...
void Mesh::RebufferData()
{
	...
	if (indices) {
		...
		glBufferSubData(GL_ELEMENT_ARRAY_BUFFER, 0,
			numVertices * sizeof(unsigned int), (void *)indices);
	}
	if (normals) {
		glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, bufferObject[NORMAL_BUFFER]);
		glBufferSubData(GL_ELEMENT_ARRAY_BUFFER, 0,
			numVertices * sizeof(Vector3), (void *)normals);
	}
	if (tangents) {
		glGenBuffers(1, &bufferObject[TANGENT_BUFFER]);
		glBindBuffer(GL_ARRAY_BUFFER, bufferObject[TANGENT_BUFFER]);
	...
	//A sneak preview of the lighting tutorials ( definately not a mistake )
	   //normals/tangents
}

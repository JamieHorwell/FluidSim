...
Fluid::Fluid(std::string name, Processor processor, bool complex, bool interactive, int width) {
...
void Fluid::updateComplexCPU() {
		//force
		double f;
		//wave speeds
		float c = 25;
		//dimensions of fluid cell
		float h = 1;
		//fixed time-step
		float t = 0.0033;
		for (int i = 0; i < this->height; ++i) {
...
void Fluid::updateInteractiveGPU()
{
...
void Fluid::setTitle()
{
	if (processorToUse == Processor::CPU && !complex) {
		...
		title = "Heterogenous Complex Fluid Representation using OpenCL";
	}
